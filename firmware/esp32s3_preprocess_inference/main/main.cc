...
ParityStats CompareGeneratedInput() {
  ...
  for (int i = 0; i < birdhouse::kFeatureElements; ++i) {
    const int diff =
        static_cast<int>(g_generated_input[i]) -
...
bool InputParityPass(const ParityStats& stats) {
  const float n =
      static_cast<float>(birdhouse::kFeatureElements);
  const float within_2_pct =
      100.0f * static_cast<float>(stats.within_2) / n;
  const float mean_abs =
      static_cast<float>(stats.sum_abs) / n;
...
bool OutputMatchesReference(int8_t output_q, bool is_bird) {
  const int diff =
      std::abs(static_cast<int>(output_q) -
               static_cast<int>(
                   birdhouse::kReferenceModelOutputInt8));
  const bool decision_match =
      (is_bird == birdhouse::kReferenceModelOutputIsBird);
...
void LogMemory(const char* stage) {
  ESP_LOGI(
      kTag,
      "%s free_internal=%u free_psram=%u largest_internal=%u",
      stage,
...
extern "C" void app_main(void) {
  ESP_LOGI(
      kTag,
      "Birdhouse optimized preprocessing-to-inference test");
  ESP_LOGI(
      kTag,
      "FFT backend: %s",
  ...
  ESP_LOGI(
      kTag,
      "Preprocess scratch bytes: %u",
      static_cast<unsigned>(
          birdhouse::AudioPreprocessScratchBytes()));
  ESP_LOGI(
      kTag,
      "Reference PCM samples: %u",
      static_cast<unsigned>(
  ...
  ESP_LOGI(
      kTag,
      "Tensor arena bytes: %d",
      birdhouse::kTensorArenaSizeBytes);

  LogMemory("before_preprocess");

  const int64_t preprocess_start_us = esp_timer_get_time();
  const bool preprocess_ok =
      birdhouse::PreprocessFloatPcmToInt8ModelInput(
          birdhouse::kReferencePcmFloat32,
          birdhouse::kReferencePcmFloat32Length,
          g_generated_input,
          birdhouse::kFeatureElements);
  const int64_t preprocess_us =
      esp_timer_get_time() - preprocess_start_us;

  ESP_LOGI(
      kTag,
      "preprocessing_time_ms: %.3f",
      static_cast<double>(preprocess_us) / 1000.0);
  LogMemory("after_preprocess");
  ...
  const float n =
      static_cast<float>(birdhouse::kFeatureElements);
  const float exact_pct =
      100.0f * static_cast<float>(stats.exact) / n;
  const float within_1_pct =
      100.0f * static_cast<float>(stats.within_1) / n;
  const float within_2_pct =
      100.0f * static_cast<float>(stats.within_2) / n;
  const float mean_abs =
      static_cast<float>(stats.sum_abs) / n;
  ...
  ESP_LOGI(
      kTag,
      "Input parity exact_match_pct: %.4f",
      static_cast<double>(exact_pct));
  ESP_LOGI(
      kTag,
      "Input parity within_1_pct: %.4f",
      static_cast<double>(within_1_pct));
  ESP_LOGI(
      kTag,
      "Input parity within_2_pct: %.4f",
      static_cast<double>(within_2_pct));
  ESP_LOGI(
      kTag,
      "Input parity mean_abs_diff_int8: %.6f",
      static_cast<double>(mean_abs));
  ESP_LOGI(
      kTag,
      "Input parity max_abs_diff_int8: %d",
      stats.max_abs);
  ESP_LOGI(
      kTag,
      "Input parity max_diff_index: %d",
      stats.max_index);
  ...
  std::copy(
      g_generated_input,
      g_generated_input + birdhouse::kFeatureElements,
      input);
  ...
  const int64_t inference_us =
      esp_timer_get_time() - inference_start_us;

  ESP_LOGI(
      kTag,
      "inference_time_ms: %.3f",
      static_cast<double>(inference_us) / 1000.0);
  LogMemory("after_inference");
  ...
  ESP_LOGI(
      ...
      "Reference p_bird: %.6f",
      static_cast<double>(
          birdhouse::kReferenceModelOutputPBird));
  ESP_LOGI(
      kTag,
      "Reference is_bird: %s",
      birdhouse::kReferenceModelOutputIsBird
          ? "true"
          : "false");
  ...
  if (OutputMatchesReference(output_q, is_bird)) {
    ...
    ESP_LOGE(
        kTag,
        "OPTIMIZED PREPROCESSING-INFERENCE TEST FAIL");
  }
}
